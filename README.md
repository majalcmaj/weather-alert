...
```bash
weather-alert$ sam logs -n StockCheckerFunction --stack-name weather-alert --tail
```
...
```bash
weather-alert$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name weather-alert
```
